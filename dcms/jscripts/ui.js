var UI = new function() {
    ...
    this.popup = function(content, settings) {
        ...
        $('#ui_modal_window_background').fadeIn(200);
    ...
    this.popup_close = function() {
        $('#ui_modal_window_background').fadeOut(100, function() {
            $('#ui_modal_window_background').remove();
        });
        $('#ui_modal_window_container').fadeOut(100, function() {
            $('#ui_modal_window_container').remove();
        });
    };
...
var Slider = function(slideobject,options) {
    ...
    // нам необходимо каждому элементу выписать его смещение относительно слайдшоу
...
(function($){
	...
	$.fn.onscroll = function(call) {
		...
		// проверяем что хэндлер окна и то что нам передано совпадают
...
(function($){
	$.fn.unexclick = function(selector) {
		if(selector == undefined) { selector = 'HTML'; 	}
		
		$(selector).data('exclick_binded','no');
		return this;
	};
...
(function($){
	$.fn.range = function(options){
		...
		var drag_bar = '<div class="drag_bar"><div class="drag_bar_line">'+
			'<div class="less_slider drag_slider"></div>'+
			'<div class="more_slider drag_slider"></div>'+
			'</div></div>';
		var drag_limits='<div class="less_value">'+options.lower+'</div>'+
						'<div class="more_value">'+options.upper+'</div>';
		this.html(drag_bar);
		
		if(options.drag_bar_position=='before') {
			this.find('div.drag_bar').before(drag_limits);
		} else {
		...
		var more_slider = {
			...
			maxOffset : function(){
				return drag.find('.drag_bar_line').width()-more_slider.width()/2;
			},
			minOffset : function(){
		...
		this.find('div.drag_slider').mousedown(function(e){
			...
			$(document).mousemove(startDrag).mouseup(stopDrag);
			return false;
		});
		
		function startDrag(e){
			var xs = more_slider.offset();
			drag_slider.offset(e.clientX-drag_slider.width()/2-drag.find('.drag_bar_line').offset().left);
			var filter_value = Math.floor( drag_slider.offset() * ( parseInt(options.upper) - parseInt(options.lower) ) / more_slider.maxOffset() ) + parseInt(options.lower);
			//console.log(filter_value);
			filter_value = Math.floor(Math.round(filter_value/step) * step);
			drag_limit.html(filter_value);
		}
