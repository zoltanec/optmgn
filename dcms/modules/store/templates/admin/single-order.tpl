	...
	<tbody data-type="element" data-object-id="<{$order->object_id()}>">
						...
						<tr>
							...
							<td class="packet_item_price">
								<div class="product_show_input">
									<div class="packet_price"><{if $prod->price}><span><{$prod->price}></span> руб.&nbsp;<{/if}><br>
								</div>
								</div>
							</td>
							<tD>x <{$product->quantity}> шт.</tD>
							<td><{if $prod->price}>= <{$product->quantity * $product->price}> руб.<{/if}></td>
						</tr>
					<{/foreach}>
				</table>
			<{/foreach}>

			<b><span class="big">Итого: </span><{$order->sum-$order->delivery_info->price}> руб.
			<{if $order->delivery_info}>
			+ <{$order->delivery_info->price}> руб (<{$order->delivery_info->name}>)  = <{$order->sum}> руб.
			<{/if}>

			<{if $order->delivery == 0 and $config['store.delivery.self_discount'] > 0}>
			(<{$config['store.delivery.self_discount']}> % скидка)
			<{/if}>
		...
		<td rowspan="2" class="order_change_status">
			<{foreach item=status key=status_key from=Store_Cart::getStatuses()}>
				<a class="order_set_<{$status_key}>" href="<{$run.me}>/update-order/ordid_<{$order->order_id}>/mode_status/status_<{$status_key}>/"><{$status}></a>
			<{/foreach}>
