...
class bbcode {	/* Описания свойств и методов смотрите в документации. */
    ...
    var $children = array(
        ...
        'strike','sub','sup','table','tt','u','url','hidden','youtube'
    );
    var $mnemonics = array();
    var $autolinks = true;
    var $is_close = false;
        ...
        foreach ($elems as $elem) {
            if ('text' == $elem['type']) {
                ...
                if (class_exists($handler)) {
                    ...
                    $tag -> tree = $elem['val'];
                    $result .= $tag -> get_html($elems = false);
                } else {
...
class bb_img extends bbcode {
    ...
    function get_html($elems = false) {
        ...
        $src = str_replace('(', '&#'.ord('(').';', $src);
        $src = str_replace(')', '&#'.ord(')').';', $src);
        return '<img class="bbimage" src="'.$src.'" '.$attr.' />';
    }
}
...
class bb_hidden extends bbcode {

        var $ends = array();
    function get_html($elems = false) {
    	$elemId = rand().rand();
       return
        '<a href="#" class="cm_hidden_text_link" onclick="return showHideBBText(\''.$elemId.'\');">[скрытый текст]</a><div id="cm_hidden_text'.$elemId.'" class="cm_hidden_text">'.parent::get_html($this -> tree).'</div>';
    }
}
...
class bb_quote extends bbcode {
    ...
    function get_html($elems = false) {
        if ($author = htmlspecialchars($this -> attrib['quote'])) {
            $author = '<div class="bb_quote_author">'.htmlspecialchars_decode($author).'</div>';
        }
...
class bb_youtube extends bbcode {
	...
	function get_html($elems = false) {
		if(preg_match('/youtu.be/', parent::get_html($this->tree))) {
			$code = substr(parent::get_html($this->tree), strrpos(parent::get_html($this->tree), '/'));
			if(strlen($code) < 4) {
				return '';
			}  else {
				$id = preg_replace('/[^A-Za-z0-9\_\-]/','', $code);
			}
		} else {
			$id = preg_replace('/(.*)watch\?v=([a-zA-Z0-9\_]+)(.*)/','\2',parent::get_html($this->tree));
		}
		return  '<object width="540" height="335">
<param name="movie" value="http://www.youtube.com/v/'.$id.'?fs=1&amp;hl=ru_RU"></param>
<param name="allowFullScreen" value="true"></param>
<param name="allowscriptaccess" value="always"></param>
<embed src="http://www.youtube.com/v/'.$id.'?fs=1&amp;hl=ru_RU" type="application/x-shockwave-flash" wmode="opaque" allowscriptaccess="always" allowfullscreen="true" width="540" height="330">
</embed>
</object>';
