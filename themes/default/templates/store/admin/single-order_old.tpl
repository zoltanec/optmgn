	...
	<tbody data-type="element" data-object-id="<{$order->object_id()}>">
		<tr>
			...
			<td rowspan="2" class="order">
				...
				<b>Пакет <{$smarty.foreach.pack.iteration}></b>
						...
						<tr>
							...
							<td class="packet_item_price">
								<div class="product_show_input">
									<div class="packet_price"><{if $prod->current_price}><span><{if $order->delivery == 0 and $config['store.delivery.self_discount'] > 0}><{floor($prod->current_price*(1-0.01*$config['store.delivery.self_discount']))}><{else}><{$prod->current_price}><{/if}></span> руб.&nbsp;<{/if}><br>
								</div>
								</div>
							</td>
							<tD>x <{$product->quantity}> шт.</tD>
							<td><{if $prod->current_price}>= <{*if $order->delivery == 0 and $config['store.delivery.self_discount'] > 0}><{$product->quantity * floor($prod->current_price*(1-0.01*$config['store.delivery.self_discount']))}><{else*}><{$product->quantity * $prod->current_price}><{*/if*}> руб.<{/if}></td>
						</tr>
					<{/foreach}>
				</table>
			<{/foreach}>

			<b><span class="big">Итого: </span>
